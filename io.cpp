...
void io_display_bag() {
    ...
    mvprintw(3, 1, "1. Pokeballs: %d", bag.num_pokeballs);
    mvprintw(4, 1, "2. Potions: %d", bag.num_potions);
    mvprintw(5, 1, "3. Revives: %d", bag.num_revives);
...
/**
int moveHits(*pokemon trainer, *pokemon pc) {

 // returns 1 if rand() mod 100 < moves.accuracy, returns 0 otherwise

 int result = trainer->pokemonMoves[move].accuracy;
...
void attack(*pokemon trainer, *pokemon pc) {

  double damage = trainer->level;

  double totalDamage = (((2*damage)/5+2) * trainer[move].power * (attack/defense) / 50) + 2 * (critical*random*stab*type);
...
void io_encounter_pokemon()
{
  ...
  mvprintw(0,0, "%s wants to fight you!", p->get_species());
  mvprintw(2,0, "Move 1: %s", p->get_move(0));
  mvprintw(3,0, "Move 2: %s", p->get_move(1));
  mvprintw(5,0, "SHINY: %s", p->is_shiny() ? "SHINY" : "NOT SHINY");
  mvprintw(6,0, "gender: %s", p->get_gender_string());
  mvprintw(7,0, "HP: %d", p->get_hp());
  mvprintw(8,0, "Attack: %d", p->get_atk());
  mvprintw(9,0, "Defense: %d", p->get_def());
  mvprintw(10,0, "Special Attack: %d", p->get_spatk());
  mvprintw(11,0, "Special Defense: %d", p->get_spdef());
  mvprintw(12,0, "Speed: %d", p->get_speed());
  mvprintw(13,0, "1. Fight  2. Bag  3. Run  4. Pokemon");
  choice = getch();
...
/*
  ...
  //mvprintw(13,0, "PRESS ANY KEY TO QUIT BATTLE");
